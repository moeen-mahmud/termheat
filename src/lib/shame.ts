...
export function shameLine(idleDays: number | null): string | null {
	if (idleDays === null) return "🦖 404 contributions found — this year's grid is a blank canvas.";
	if (idleDays === 0) return "✨ Shipped today. Zero shame detected.";
	if (idleDays === 1) return "🐌 Quiet for a day — the keyboard misses you.";
	if (idleDays <= 3) return `🐌 You haven't pushed in ${idleDays} days. The grid is cooling…`;
	if (idleDays <= 6) return `🥶 ${idleDays} days idle. Your green squares are fading to gray.`;
	if (idleDays <= 13) return `🕸️ ${idleDays} days. Cobwebs are forming on \`git push\`.`;
	if (idleDays <= 29) return `💀 ${idleDays} days of silence. \`git blame\` finds only ghosts.`;
