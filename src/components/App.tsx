...
export function App({
  ...
  const anim = useAnimation(interactive);
    ...
    <Box flexDirection="column" paddingY={1}>
      ...
      {days === null || visibleHeatmap === null ? (
        <Box marginLeft={2}>
          {error === null ? (
            <Text dimColor>
              {SPINNER[anim.tick % SPINNER.length]} fetching {username}
              &rsquo;s year…
            </Text>
          ) : (
        ...
        <>
          <Heatmap
            ...
            anim={anim}
            revealFrom={interactive ? revealFrom : null}
          />
          <StatsBar
            days={days}
