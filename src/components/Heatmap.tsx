...
export function Heatmap({ heatmap, theme, streakDates, anim, revealFrom, ascii }: HeatmapProps) {
	const { weeks } = heatmap;
	const chars = ascii ? ASCII_CHARS : CHARS;
	const reveal = revealFrom === null ? 1 : revealProgress(anim.tick, revealFrom);
	// Whole grid gently breathes between 72% and 100% brightness.
	const breathe = ANIMATION_BREATHE_EXP_LEFT + ANIMATION_BREATHE_EXP_RIGHT * anim.breathe;
				...
				<Box key={weekday}>
					...
					{weeks.map((week, w) => {
						...
						if (fire !== undefined) {
							...
							const ramp = FIRE_RAMP[Math.floor((fire / streakDates.length) * FIRE_RAMP.length)]!;
							const flicker =
								FLICKER_LEFT +
								FLICKER_RIGHT * Math.sin(anim.tick * FLICKER_FREQUENCY + fire * FLICKER_PHASE_OFFSET);
