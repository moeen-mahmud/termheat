 ...
 */
export async function loadConfig(path: string = configPath()): Promise<TermheatConfig> {
	let raw: string;
	try {
...
export async function saveConfig(config: TermheatConfig, path: string = configPath()): Promise<void> {
	await writeFile(path, `${JSON.stringify(sanitize({ ...config }), null, 2)}\n`, "utf8");
}

function sanitize(raw: Record<string, unknown>): TermheatConfig {
	const config: TermheatConfig = {};
	if (typeof raw.username === "string" && raw.username.length > 0) config.username = raw.username;
	if (THEMES.includes(raw.theme as ThemeName)) config.theme = raw.theme as ThemeName;
